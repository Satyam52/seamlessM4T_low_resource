...
class UnitYModel(EncoderDecoderModel):
    ...
    def __init__(
        ...
        if text_encoder is not None:
            ...
            for param in text_encoder_frontend.parameters():
                param.requires_grad = False
                
            for name, module in text_encoder.named_modules():
                # Freeze module which does not contain "language_adaptor" in its name
                if "text_language_adaptor" not in name: #or "ffn_layer_norm" not in name:
        ...
        if speech_encoder_frontend is not None:
            for param in speech_encoder_frontend.parameters():
                param.requires_grad = False
        ...
        if text_decoder is not None:
            ...
            #     param.requires_grad = False
            for param in text_decoder_frontend.parameters():
                param.requires_grad = False
            for param in final_proj.parameters():
                param.requires_grad = False
            
            # print all modules inside decoder
            for name, module in text_decoder.named_modules():
                # Freeze module which does not contain "language_adaptor" in its name
                if "text_language_adaptor" not in name:
                    for param in module.parameters():
                        param.requires_grad = False
                else:
                    for param in module.parameters():
        ...
        if t2u_model is not None:
            for param in t2u_model.parameters():
                param.requires_grad = False
